...
- **basemap** — `styleTemplate`: `satellite` | `vector` | `white` | `dark`; `projection`: `globe` | `mercator`; `buildings3d`. See [fx-world.md](fx-world.md).
...
| `world` | terrain / fog / light (one set of values). [fx-world.md](fx-world.md) |
| `geojson` | a drawn layer (polygon/line/points) — references a DataAsset + drawingOption |
| `marker` | on-map labels/icons. [markers.md](markers.md) |
...
```
- Polygon → animated fill + light-running border. LineString → draw-on via `trimLineEnd` × progress. Points → auto POI markers.
...
- **drawingOptions** — per-layer style: `fill{addFill,fillColor:{r,g,b,a},opacity}`, `stroke{addStroke,strokeColor,strokeWidth}`.
